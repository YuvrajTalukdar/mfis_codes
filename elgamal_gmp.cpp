...
int main()
{
    ...
    else if(option==2)//ok check
    {
        ...
        for(int a=0;a<encoded_text.length();a++)
        {
            ...
            if(count==msg_shunk_size)
            {
                ...
                mpz_t ssm_result_mpz,shunk_mpz,p_mpz;
                mpz_init(ssm_result_mpz);
                mpz_init_set_str(ssm_result_mpz,ssm_result.convert_to<string>().c_str(),10);
                mpz_init_set_str(shunk_mpz,string_to_number(shunk).convert_to<string>().c_str(),10);
                mpz_mul(ssm_result_mpz,ssm_result_mpz,shunk_mpz);
                //ssm_result=ssm_result*string_to_number(shunk);//gmp_
                mpz_init_set_str(p_mpz,p.convert_to<string>().c_str(),10);
                mpz_mod(ssm_result_mpz,ssm_result_mpz,p_mpz);
                //ssm_result=ssm_result%p;//gmp_
                //cout<<"\nresult="<<ssm_result;
                encrypted_msg_chunk_vec_c2.push_back(string_to_number(mpz_get_str(NULL,10,ssm_result_mpz)));
                shunk.clear();
            }
        }
        if(shunk.length()>0)
    ...
    else if(option==3)
    {
        ...
        for(int a=0;a<encrypted_msg_chunk_vec_c2.size();a++)
        {
            ...
            mpz_t c1_power_d_inv_mpz,encrypted_msg_chunk_vec_c2_mpz,mul_mpz,mod_mpz,p_mpz;
            mpz_init_set_str(c1_power_d_inv_mpz,c1_power_d_inv.convert_to<string>().c_str(),10);
            mpz_init_set_str(encrypted_msg_chunk_vec_c2_mpz,encrypted_msg_chunk_vec_c2.at(a).convert_to<string>().c_str(),10);
            mpz_init_set_str(p_mpz,p.convert_to<string>().c_str(),10);
            mpz_init(mul_mpz);
            mpz_mul(mul_mpz,c1_power_d_inv_mpz,encrypted_msg_chunk_vec_c2_mpz);
            mpz_init(mod_mpz);
            mpz_mod(mod_mpz,mul_mpz,p_mpz);
            //temp=((c1_power_d_inv*encrypted_msg_chunk_vec_c2.at(a))%p).convert_to<string>();//gmp_
            temp=mpz_get_str(NULL,10,mod_mpz);
